...
N = 2173;
kernelSamps = [500, 500];                            % sampling size for kernel logistic regressio
...
% % % %             % In this implementation we can use the same V's for both the
% % % %             % modalities
% % % %             % also you need a single W
% % % %             lambdas = [1 1 1]; gamma = 1; alpha = 1;
% % % %             [U1, U2, U3, V, V3, W] = solveUCMFH_devraj6_proj(trainXs{1}, trainXs{2}, trainXs{3}, lambdas, gamma, alpha, bit);
% % % %             % hash codes are as follows
% % % %             A = sign((V3).');            
            
            
            % In this implementation you can make the V's same
            % and also the W's same 
            % and also the U's same            
%             option = 1; % pca
%             option = 2; % cca
            option = 6; % random initializations
            lambdas = [1 1 1]; gamma = 1; alpha = 1;
            [U, U3, V, V3, W] = solveUCMFH_devraj7_proj(trainXs{1}, trainXs{2}, trainXs{3}, lambdas, gamma, alpha, bit, option);
            % hash codes are as follows
            A = sign((V3).');
            
            % Evaluating the Quality of Learnt Hash Codes for Training Set
            trEv = trainEval2(tr_labels, A, A);

function [U1, U2, U3, V1, V2, V3, W1, W2] = solveUCMFH_devraj3(X1, X2, X3, lambdas, gamma, alphas, bits)
...
for t=1:niter    
    ...
    U1 = X1*(V1.') / (V1 * V1.' + gamma\lambdas(1)*eye(bits));
    U2 = X2*(V2.') / (V2 * V2.' + gamma\lambdas(2)*eye(bits));
    ...
    %update W1 and W2
    W1 = V3 * V1.' / (V1 * V1.' + gamma\alphas(1) * eye(bits));
    W2 = V3 * V2.' / (V2 * V2.' + gamma\alphas(2) * eye(bits));
